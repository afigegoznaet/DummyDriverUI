...
private:
	...
	//==========================================================================
	class ItemComponent : public Component, public SettableTooltipClient {
	public:
		ItemComponent(Item it, bool shouldBeTicked,
...
public:
	...
	/** Creates a search-box.
		...
		@param componentName    the name to set for the component (see
	   Component::setName())
	*/
	...
	*/
	...
	/** Adds an item to be shown in the drop-down list.
		...
		@param newItemId        an associated ID number that can be set or
	   retrieved - see getSelectedId() and setSelectedId(). Note that this value
	   can not be 0!
		@see setItemEnabled, addSeparator, addSectionHeading, getNumItems,
	   getItemText, getItemId
	*/
	...
	*/
	...
	/** Returns a Value object that can be used to get or set the selected
	   item's ID.

		You can call Value::referTo() on this object to make the combo box
	   control another Value object.
	*/
	...
	/** Sets one of the items to be the current selection.
		...
		@param notification     determines the type of change notification that
	   will be sent to listeners if the value changes
	...
	*/
	void
	setSelectedItemIndex(int			  newItemIndex,
						 NotificationType notification = sendNotificationAsync);
	...
	*/
	...
	/** Programmatically opens the text editor to allow the user to edit the
	   current item.
	...
	*/
	...
	/**
		...
		You can register a SearchBox::Listener with a SearchBox using the
	   SearchBox::addListener() method, and it will be called when the selected
	   item in the box changes.
	...
	*/
	...
	/** Sets the message to show when there are no items in the list, and the
	   user clicks on the drop-down box.

		By default it just says "no choices", but this lets you change it to
	...
	*/
	...
	/** This can be used to allow the scroll-wheel to nudge the chosen item.
		By default it's disabled, and I'd recommend leaving it disabled if
	   there's any chance that the control might be inside a scrollable list or
	   viewport.
	*/
